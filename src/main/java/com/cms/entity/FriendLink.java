...
public class FriendLink extends BaseFriendLink<FriendLink> {
	...
	public Page<FriendLink> findPage(String name,Integer pageNumber,Integer pageSize){
		...
		return paginate(pageNumber, pageSize, "select *", "from mzlt_friend_link where 1=1 "+filterSql+orderBySql);
	}
	 ...
	 * @return 友情链接
	 */
	public List<FriendLink> findList(String orderBy,Integer start,Integer count) {
		String orderBySql = "";
		...
		return find("select * from mzlt_friend_link where 1=1 "+orderBySql+countSql);
	}
	...
	public List<FriendLink> findList(String type) {
		...
		String orderBySql = DBUtils.getOrderBySql("createDate desc");
		return find("select * from mzlt_friend_link where 1=1 "+filterSql+orderBySql);
	}
}
