...
public class ArticleCategory extends BaseArticleCategory<ArticleCategory> {
   ...
   /**
    ...
    public List<Article> getArticles(){
        if(articles == null){
            articles = new Article().dao().find("select * from mzlt_article where articleCategoryId = ?",getId());
        }
        return articles;
    }
    ...
    public List<ArticleCategory> getChildren() {
        if(children == null){
            children = find("select * from mzlt_article_category where parentId=?",getParentId());
        }
        return children;
    }
   ...
   /**
    ...
    public List<ArticleCategory> findRoots(Integer count){
        ...
        return find("select * from mzlt_article_category where parentId is null "+orderBySql+countSql);
    }
    ...
    public List<ArticleCategory> findParents(Long articleCategoryId, boolean recursive, Integer count){
        ...
        if(recursive){
            ...
            return find("select * from mzlt_article_category where id in ("+StringUtils.join(articleCategory.getParentIds(), ",")+") "+orderBySql+countSql);
        }else{
            return find("select * from mzlt_article_category where id = ? ",findById(articleCategoryId).getParentId());
        }
    }
    ...
    public List<ArticleCategory> findChildren(Long articleCategoryId,boolean recursive,Integer count){
        if(recursive){
            ...
            if(articleCategoryId!=null){
                articleCategories = find("select * from mzlt_article_category where treePath like ? "+orderBySql+countSql,"%,"+articleCategoryId+",%");
            }else{
                articleCategories = find("select * from mzlt_article_category where 1=1 "+orderBySql+countSql);
            }
            sort(articleCategories);
            return articleCategories;
        }else{
            String orderBySql = DBUtils.getOrderBySql("sort asc");
            return find("select * from mzlt_article_category where parentId = ? "+orderBySql,articleCategoryId);
        }
    }
