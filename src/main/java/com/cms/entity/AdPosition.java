...
public class AdPosition extends BaseAdPosition<AdPosition> {
	...
	public Page<AdPosition> findPage(String name,Integer pageNumber,Integer pageSize){
		...
		return paginate(pageNumber, pageSize, "select *", "from mzlt_ad_position where 1=1 "+filterSql+orderBySql);
	}
    ...
    public List<Ad> getAds(){
        if(ads == null){
            ads = new Ad().dao().find("select * from mzlt_ad where adPositionId=?",getId());
        }
        return ads;
    }
}
