...
public class Area extends BaseArea<Area> {
   ...
   /**
    ...
    public List<Area> getChildren() {
        if(children == null){
            children = find("select * from mzlt_area where parentId=?",getId());
        }
        return children;
    }
	...
	public List<Area> findRoots(Integer count){
		...
		return find("select * from mzlt_area where parentId is null "+orderBySql+countSql);
	}
	...
	public List<Area> findParents(Long areaId, boolean recursive, Integer count){
		...
		if(recursive){
			...
			return find("select * from mzlt_area where id in ("+StringUtils.join(area.getParentIds(), ",")+") "+orderBySql+countSql);
		}else{
			return find("select * from mzlt_area where id = ? ",findById(areaId).getParentId());
		}
	}
	...
	public List<Area> findChildren(Long areaId,boolean recursive,Integer count){
		if(recursive){
			...
			if(areaId!=null){
				productCategories = find("select * from mzlt_area where treePath like ? "+orderBySql+countSql,"%,"+areaId+",%");
			}else{
				productCategories = find("select * from mzlt_area where 1=1 "+orderBySql+countSql);
			}
			sort(productCategories);
			return productCategories;
		}else{
			String orderBySql = DBUtils.getOrderBySql("sort asc");
			return find("select * from mzlt_area where parentId = ? "+orderBySql,areaId);
		}
	}
